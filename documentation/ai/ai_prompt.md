...
3. **General README Explanation:** You must update the specific assignment's `README.md` to include:
    - **Do Not Touch Original Text:** You MUST preserve the exact initial text/specs the user pasted into the README completely untouched. DO NOT format it, do not add markdown headers above it, and do not remove it. It must remain raw at the very top.
    - **Summarization:** Add a summarized, yet detailed enough explanation of what the assignment is (derived directly from the initial state of the README). This should be appended *below* the original raw text.
...
- **User Setup:** The user will manually create the assignment folder directly inside the `assignments` folder, and manually add the specification `README.md` inside it.
...
- **Initial Setup:** You go read the `README.md` inside that folder. The summary of the assignment must be derived from this initial state of the README. **DO NOT touch the original text.** Keep it exactly the way the user saved it, completely unformatted and raw at the top. Update the README by appending your summarized yet detailed enough explanation (and technical requirements) *below* the original text. Ensure you keep the original link to the assignment intact.
...
- **Folder Pathing:** Ensure the link in the index table correctly points to the assignment folder (e.g., `./assignments/02-ai-workflow-audit-and-tool-setup`).
